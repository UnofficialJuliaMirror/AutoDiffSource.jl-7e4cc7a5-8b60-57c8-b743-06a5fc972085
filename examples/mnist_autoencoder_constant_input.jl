...
# by convention argument #6 will not be differentiated
@δ function autoencoder(We1, We2, Wd, b1, b2, input_const)
    firstLayer = sigmoid(We1 * input_const + b1)
    encodedInput = sigmoid(We2 * firstLayer + b2)
    reconstructedInput = sigmoid(Wd * encodedInput)
end

# by convention argument #6 will not be differentiated
@δ function autoencoderError(We1, We2, Wd, b1, b2, input_const)
    reconstructedInput = autoencoder(We1, We2, Wd, b1, b2, input_const)
    return sum((input_const - reconstructedInput).^2)
end

# to test we need a function with 5 arguments
const input_const = randn(3)
@δ test_autoencoderError(We1, We2, Wd, b1, b2) = autoencoderError(We1, We2, Wd, b1, b2, input_const)
@assert checkdiff(test_autoencoderError, δtest_autoencoderError, randn(3,3), randn(3,3), rand(3,3), randn(3), randn(3))
